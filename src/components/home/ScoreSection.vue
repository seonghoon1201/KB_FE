<template>
  <div class="bg-white p-4 rounded-xl border">
    ...
    >
      ...
      <p class="text-xs text-[#A48BFF] font-medium">
        {{ score.percent.toFixed(1) }}% 달성
      </p>
      ...
      <!-- 버튼 -->
      <PrimaryButton customClass="bg-[#A48BFF] mt-2" @click="$emit('recalculate')">
        📝 가점 다시 계산하기
      </PrimaryButton>
    </div>
    ...
    >
      ...
      <PrimaryButton customClass="bg-[#A48BFF] mt-2" @click="$emit('calculate')">
        📝 가점 계산하기
      </PrimaryButton>
    </div>
  </div>
